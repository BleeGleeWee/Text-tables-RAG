...
from citation_verifier import VerificationResult,verify_citations
...
class RAGChain:
    ...
    # Public entry point
    # ──────────────────────────────────────────────────────────────────
    def answer(self,query: str,chat_history: Optional[List[dict]] = None,) -> RAGResponse:
        ...
        # ── 1b. Cache check (after rewrite, before any heavy work) ──

        if self._cache is not None:

            cached = self._cache.get(rewrite.rewritten_query)

            if cached is not None:

                # Note in stats that this was a cache hit. We replace the

                # rewrite field with the fresh one so any history-aware

                # rewriting from THIS call's context is reflected, not

                # whatever rewrite produced the original cached entry.

                cached.rewrite = rewrite

                cached.retrieval_stats = {
        ...
        if not refused and not answer_text.startswith("I encountered an error"):

            verification = verify_citations(answer_text, reranked)

            final_answer = verification.cleaned_answer
        ...
        # ── 7. Store in cache (skips refusals/errors internally) ──

        if self._cache is not None:

            self._cache.set(rewrite.rewritten_query, response)
...
if __name__ == "__main__":
    ...
    test_cases = [
        ...
        # },
        {
            ...
            "query": "What does the 2025 report say about land "
    ...
    for case in test_cases:
        ...
        if response.verification:
            ...
            print(f"Verification: clean={v.is_clean}  "

                  f"cited={len(v.valid_ids)}  "

                  f"hallucinated={len(v.hallucinated_ids)}  "

                  f"unused={len(v.unused_sources)}")
        print(f"\nAnswer:\n{response.answer}\n")
